    ...
    for(int f = 0; f < fil; f++){
        for(int c = 0; c < fil; c++){
          ...
          for(int j = 0; j < iteraciones;j++, h+=4){
            x1 += matriz1[f][h]*matriz2[c][h];
            x2 += matriz1[f][h+1]*matriz2[c][h+1];
            x3 += matriz1[f][h+2]*matriz2[c][h+2];
            x4 += matriz1[f][h+3]*matriz2[c][h+3];
          ...
          for(h=iteraciones*4;h<fil;h++){
            ...
            resultado[f][c] += matriz1[f][h]*matriz2[c][h];
          }
